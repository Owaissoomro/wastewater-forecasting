...
# Optional utilities (config, plotting, provenance, seeds); degrade gracefully if absent
...
class StageContext:
    ...
    NOTE: LaTeX and Parquet are removed. Outputs are:
      - CSV for metrics/tables
      - PDF+PNG for figures
      - JSONL logs
      - bundle.json with manifest
    """
    ...
    def _normalize_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        r = dict(record)
        # historical bug: payload passed as level=dict(...)
    ...
    def write_metric(self, name: str, data: Any) -> Tuple[Path, Optional[Path]]:
        """
        CSV-only metric writer. Returns (csv_path, None).
        """
        ...
        self._record_file(mirrored_csv, kind="metric(run)",
                          extra={"format": "csv", "rows": int(df.shape[0]), "cols": int(df.shape[1])})
        return csv_path, None  # parquet removed

    def write_table(
        self,
        name: str,
        data: Any,
        latex_caption: Optional[str] = None,  # kept for API compatibility, ignored
        latex_label: Optional[str] = None,    # kept for API compatibility, ignored
    ) -> Tuple[Path, Optional[Path]]:
        """
        CSV-only table writer (LaTeX removed). Returns (csv_path, None).
        """
        df = _to_dataframe(data)
        base = _sanitize_name(name)
        ...
        self._record_file(mirrored_csv, kind="table(run)",
                          extra={"format": "csv", "rows": int(df.shape[0]), "cols": int(df.shape[1])})
        return csv_path, None  # tex removed
    ...
    def close(self, inputs: Optional[List[str]] = None, notes: Optional[str] = None) -> None:
        ...
        # Log BEFORE closing files (fixes previous ordering bug)
...
class RunContext:
    ...
    Simplified: no config validation call, deterministic seeds, latest pointer.
    """
    ...
    def __init__(self, cfg: Mapping[str, Any], run_id: Optional[str] = None) -> None:
        ...
        # Effective config (no hard dependency on utils.config)
        try:
            self.cfg: Dict[str, Any] = dict(cfg)
        except Exception:
            self.cfg = json.loads(json.dumps(cfg))  # very defensive fallback
